// index page 로딩하자마자 지역 설정
let areaUrl = "https://apis.data.go.kr/B551011/KorService1/areaCode1?serviceKey=sO3MQCUWa9INBB6AYWIl%2FKCaQ0R6SzsOKAPd28hj5GnxCX1tWVlBLSG6zvN9Ep1hBiQzzRkbV%2FOfS3KBUYxOGw%3D%3D&numOfRows=20&pageNo=1&MobileOS=ETC&MobileApp=AppTest&_type=json";
...
function makeOption(data) {
  ...
  areas.forEach((area) => {
    ...
    opt.setAttribute("class", "dropDonwItem");
    opt.innerHTML = area.name;
    sel.appendChild(opt);
    opt.addEventListener("click",function(){
      document.querySelector(".textBox").value = opt.innerText;
      sessionStorage.setItem("area-code", area.code);
...
container.addEventListener('scroll', function(){
  ...
  containerList[1].style.background=`linear-gradient(rgba(0, 0, 0, ${nColor}), rgba(0, 0, 0, ${nColor})),url(./assets/bg1.jpg)`;
